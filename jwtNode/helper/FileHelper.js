...
const multipart = multer({
    ...
    storage: multer.diskStorage({
        ...
        destination: (req,file,callback)=> {
            ...
            //윈도우 환경을 고려하여 역슬래시를 슬래시로 변경하는 처리추가
        ...
        filename: (req,file,callback)=> {
            ...
            //구성된 최종 업로드 정보를 클라이언트에게 응답결과로 돌려주기 위해 req객체에게 추가
            //구성된 최종 업로드 정보를 클라이언트에게 응답결과로 돌겨주기 위해 req객체에게 추가
    ...
    fileFilter: (req,file,callback)=> {
        ...
        //확장자 제한이 있을 경우 필터링
        if(process.env.UPLOAD_FILE_FILETER !== undefined) {
            //png|jpg|gif.indexOf('png')의 처리결과가 찾지 못했다면?
            if (process.env.UPLOAD_FILE_FILETER.indexOf(extName) == -1) {
                const err = new Error();
                err.result_code = 500;
                err.result_msg = process.env.UPLOAD_FILE_FILETER.replaceAll('|',',')+'형식만 업로드 가능합니다.';
                return callback(err);
            }
        }
        callback(null,true);
...
const createThumbnail = file => {
    ...
    size.map(async(v,i)=> {
        ...
        //썸네일 정보를 width를  key로 갖는 json형태로 추가하기 위해 key이름 생성
        const key = v+'w';
